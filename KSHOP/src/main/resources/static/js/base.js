...
const idCustomer = _$('#idCustomer')
const selectedProduct = '/images/selected-variant-indicator.svg'
...
function search_by_name() {
    var inputName = _$("#search-product-js").value
...
const loadCategoryLevel0 = (data) => {
    let wrapCategoryLevel0 = _$('.category-level-0')
    let html = ``
    data.forEach(e => {
        html += /*html*/ `<option >${e.category_name}</option>`
    })
    wrapCategoryLevel0.innerHTML = html
}

//category
http.get("http://localhost:8080/ShopCategories/get/level/0")
    .then(data => {
        loadCategoryLevel0(data)
        setDataCategories(data, 0)
    })
    .catch(err => console.log(err))

http.get("http://localhost:8080/ShopCategories/get/level/1")
    .then(data => setDataCategories(data, 1))
    .catch(err => console.log(err))

http.get("http://localhost:8080/ShopCategories/get/level/2")
    .then(data => setDataCategories(data, 2))
    .catch(err => console.log(err))

var iconCategories = ["w-icon-tshirt2", "w-icon-home", "w-icon-electronics", "w-icon-furniture", "w-icon-heartbeat", "w-icon-gift", "w-icon-gamepad", "w-icon-ice-cream", "w-icon-ios", "w-icon-camera", "w-icon-ruby"]
...
<div class="banner-fixed menu-banner menu-banner2">
    <figure>
        <img src="/images/banner-2.jpg" alt="Menu Banner" width="235" height="347">
    </figure>
    <div class="banner-content">
        <div class="banner-price-info mb-1 ls-normal">Giảm giá đến
            <strong class="text-primary text-uppercase">20%</strong>
        </div>
        <h3 class="banner-title ls-normal">Hot Sales</h3>
        <a href="shop-grid-4cols.html" class="btn btn-dark btn-sm btn-link btn-slide-right btn-icon-right">
            Mua ngay<i class="w-icon-long-arrow-right"></i>
        </a>
    </div>
</div>
</li>`
const genderCategories = (data) => {
    let [dataLv0, dataLv1, dataLv2] = data

    let html = ``

    dataLv0.forEach((e, i) => { // lv0
        let hasSubMenu = false
        let htmlChild = ``

        dataLv1.forEach(eLv1 => { // lv1
            let htmlChild2 = ``

            dataLv2.forEach(eLv2 => { // lv2
                if (eLv1.id == eLv2.parentId) {
                    htmlChild2 += `<li><a href="shop-grid-4cols.html">${eLv2.category_name}</a></li>`
                }
            })

            if (e.id == eLv1.parentId) {
                hasSubMenu = true
                htmlChild += `<li>
                    <h4 class="menu-title">${eLv1.category_name}</h4>
                    <hr class="divider">
                    <ul>
                        ${htmlChild2}
                    </ul>
        ...
        html += `<li class="${hasSubMenu ? 'has-submenu': ''}">
            <a href="shop-grid-4cols.html">
                <i class="${iconCategories[i]}"></i>${e.category_name}
            </a>
            <ul class="megamenu category-level-1">
                ${htmlChild}
                ${i == 0 ? thoiTrang : ''}
            </ul>
        </li>`
    ...
    let category_menu = _$('.category-menu')

    category_menu.innerHTML = html
}

setTimeout(() => {
    genderCategories(dataCategories)
}, 1000)
...
function getDistrict(province_id, need) {
    let data = {
    ...
    http.post(url, data, header)
        .then(data => {
            data.data.forEach(e => {
                ...
                if (e.DistrictName == need || e.DistrictID == need) {
                    dataDistrictRoot = e
                }
            })
        })
...
function getWard(district_id, need) {
    let data = {
    ...
    http.post(url, data, header)
        .then(data => {
            data.data.forEach(e => {
                ...
                if (e.WardName == need || e.WardCode == need) {
                    dataWardRoot = e
                }
            })
        })
